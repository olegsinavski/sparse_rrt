    ...
    double solution_node_diameter, double solution_line_width, double tree_line_width)
{
    ...
    visualize_edge(root,doc,dimensions, tree_line_width);
	...
	visualize_solution_path(doc, dimensions, solution_line_width);
...
void planner_t::visualize_nodes(int image_counter, int image_width, int image_height, double node_diameter, double solution_node_diameter)
{
	std::stringstream s;
    ...
    {
	    visualize_node(sorted_nodes[i],doc,dimensions, node_diameter);
	}
	...
	visualize_solution_nodes(doc, dimensions, solution_node_diameter);
    system->visualize_obstacles(doc,dimensions);
...
void planner_t::visualize_edge(tree_node_t* node, svg::Document& doc, svg::Dimensions& dim, double tree_line_width)
{
	...
	for (std::list<tree_node_t*>::iterator i = node->children.begin(); i != node->children.end(); ++i)
	{
		...
		visualize_edge(*i, doc, dim, tree_line_width);

	}

}

void planner_t::visualize_node(tree_node_t* node, svg::Document& doc, svg::Dimensions& dim, double node_diameter)
{
...
void planner_t::visualize_solution_nodes( svg::Document& doc, svg::Dimensions& dim, double solution_node_diameter)
{
